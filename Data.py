...
datasave={"ON":False,"OFF":False,"ID":0,"user1":"","user2":"","promptNotF":False,"LimitReached":False,"alarm":"1","numberRep":1,"voice":0,"AlarmText":"intruder!"}
def load():#this method is used to load the data from the txt file
    json_file = open("C:/Users/appli/PycharmProjects/IntruderFaceDetection/datasave.txt")
    name = json.load(json_file)
    json_file.close()
    return name
def save(data):#this method is used to save the data in the text file
    file = open('C:/Users/appli/PycharmProjects/IntruderFaceDetection/datasave.txt', 'w')
    json.dump(data, file)
    file.close()
